...
When using the [Permission Model](/nodejs/api/permissions#permission-model), the process will not be able to spawn any child process by default. Attempts to do so will throw an `ERR_ACCESS_DENIED` unless the user explicitly passes the `--allow-child-process` flag when starting Node.js.
...
When using the [Permission Model](/nodejs/api/permissions#permission-model), the process will not be capable of creating any WASI instances by default. For security reasons, the call will throw an `ERR_ACCESS_DENIED` unless the user explicitly passes the flag `--allow-wasi` in the main Node.js process.
...
It is possible to run code containing inline types unless the [`--no-experimental-strip-types`](/nodejs/api/cli#--no-experimental-strip-types) flag is provided.
...
### `--experimental-loader=module` {#--experimental-network-inspection}
...
To allow polyfills to be added, [`--require`](/nodejs/api/cli#-r---require-module) and [`--import`](/nodejs/api/cli#--importmodule) both run before freezing intrinsics.
...
Disable experimental type-stripping for TypeScript files. For more information, see the [TypeScript type-stripping](/nodejs/api/typescript#type-stripping) documentation.
...
A test reporter to use when running tests. See the documentation on [test reporters](/nodejs/api/test#test-reporters) for more details.
...
Regenerates the snapshot files used by the test runner for [snapshot testing](/nodejs/api/test#snapshot-testing).
...
### `--tls-min-v1.2` {#--trace-deprecation}
...
### `NODE_COMPILE_CACHE=dir` {#node_disable_colors=1}
...
```
### `NO_COLOR=&lt;any&gt;` {#ssl_cert_file=file}

[`NO_COLOR`](https://no-color.org/)  is an alias for `NODE_DISABLE_COLORS`. The value of the environment variable is arbitrary.

### `OPENSSL_CONF=file` {#tz}

**Added in: v6.11.0**

Load an OpenSSL configuration file on startup. Among other uses, this can be used to enable FIPS-compliant crypto if Node.js is built with `./configure --openssl-fips`.

If the [`--openssl-config`](/nodejs/api/cli#--openssl-configfile) command-line option is used, the environment variable is ignored.

### `SSL_CERT_DIR=dir` {#uv_threadpool_size=size}
...
### `SSL_CERT_FILE=file` {#useful-v8-options}
...
### `TZ` {#--abort-on-uncaught-exception_1}
...
While Node.js does not support all of the various [ways that `TZ` is handled in other environments](https://www.gnu.org/software/libc/manual/html_node/TZ-Variable), it does support basic [timezone IDs](https://en.wikipedia.org/wiki/List_of_tz_database_time_zones) (such as `'Etc/UTC'`, `'Europe/Paris'`, or `'America/New_York'`). It may support a few other abbreviations or aliases, but these are strongly discouraged and not guaranteed.

```bash [BASH]
$ TZ=Europe/Dublin node -pe "new Date().toString()"
Wed May 12 2021 20:30:48 GMT+0100 (Irish Standard Time)
```
### `UV_THREADPOOL_SIZE=size` {#--disallow-code-generation-from-strings_1}
...
## Useful V8 options {#--enable-etw-stack-walking}
...
### `--abort-on-uncaught-exception` {#--expose-gc_1}

### `--disallow-code-generation-from-strings` {#--harmony-shadow-realm}

### `--enable-etw-stack-walking` {#--interpreted-frames-native-stack}

### `--expose-gc` {#--jitless_1}

### `--harmony-shadow-realm` {#--max-old-space-size=size-in-mib}

### `--interpreted-frames-native-stack` {#--max-semi-space-size=size-in-mib}

### `--jitless` {#--perf-basic-prof}

### `--max-old-space-size=SIZE` (in MiB) {#--perf-basic-prof-only-functions}
...
```
### `--max-semi-space-size=SIZE` (in MiB) {#--perf-prof}

Sets the maximum [semi-space](https://www.memorymanagement.org/glossary/s#semi.space) size for V8's [scavenge garbage collector](https://v8.dev/blog/orinoco-parallel-scavenger) in MiB (mebibytes). Increasing the max size of a semi-space may improve throughput for Node.js at the cost of more memory consumption.
...
```
### `--perf-basic-prof` {#--perf-prof-unwinding-info}

### `--perf-basic-prof-only-functions` {#--prof_1}

### `--perf-prof` {#--security-revert}

### `--perf-prof-unwinding-info` {#--stack-trace-limit=limit}
