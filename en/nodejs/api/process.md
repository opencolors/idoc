...
The listener callback function is invoked with the value of [`process.exitCode`](/nodejs/api/process#processexitcode_1) passed as the only argument.
...
If the Node.js process is spawned with an IPC channel (see the [Child Process](/nodejs/api/child_process) and [Cluster](/nodejs/api/cluster) documentation), the `'disconnect'` event will be emitted when the IPC channel is closed.
...
The effect of calling `process.disconnect()` is the same as calling [`ChildProcess.disconnect()`](/nodejs/api/child_process#subprocessdisconnect) from the parent process.
...
In this example, an `Error` object is generated internally by `process.emitWarning()` and passed through to the [`'warning'`](/nodejs/api/process#event-warning) handler.
...
This is the legacy version of [`process.hrtime.bigint()`](/nodejs/api/process#processhrtimebigint) before `bigint` was introduced in JavaScript.
...
The `process.noDeprecation` property indicates whether the `--no-deprecation` flag is set on the current Node.js process. See the documentation for the [`'warning'` event](/nodejs/api/process#event-warning) and the [`emitWarning()` method](/nodejs/api/process#processemitwarningwarning-type-code-ctor) for more information about this flag's behavior.
...
This API is available through the [`--permission`](/nodejs/api/cli#--permission) flag.

`process.permission` is an object whose methods are used to manage permissions for the current process. Additional documentation is available in the [Permission Model](/nodejs/api/permissions#permission-model).
...
The value `'android'` may also be returned if the Node.js is built on the Android operating system. However, Android support in Node.js [is experimental](https://github.com/nodejs/node/blob/HEAD/BUILDING.md#android).
...
## `process.ref(maybeRefable)` {#processrelease}
...
- `sourceUrl` [\<string\>](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#String_type) an absolute URL pointing to a *<code>.tar.gz</code>* file containing the source code of the current release.
- `headersUrl`[\<string\>](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#String_type) an absolute URL pointing to a *<code>.tar.gz</code>* file containing only the source header files for the current release. This file is significantly smaller than the full source file and can be used for compiling Node.js native add-ons.
- `libUrl` [\<string\>](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#String_type) | [\<undefined\>](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#Undefined_type) an absolute URL pointing to a *<code>node.lib</code>* file matching the architecture and version of the current release. This file is used for compiling Node.js native add-ons. *This property is only present on
...
## `process.report` {#processreportcompact}
...
`process.report` is an object whose methods are used to generate diagnostic reports for the current process. Additional documentation is available in the [report documentation](/nodejs/api/report).

### `process.report.compact` {#processreportdirectory}
...
### `process.report.directory` {#processreportfilename}
...
### `process.report.filename` {#processreportgetreporterr}
...
### `process.report.getReport([err])` {#processreportreportonfatalerror}
...
Additional documentation is available in the [report documentation](/nodejs/api/report).

### `process.report.reportOnFatalError` {#processreportreportonsignal}
...
### `process.report.reportOnSignal` {#processreportreportonuncaughtexception}
...
### `process.report.reportOnUncaughtException` {#processreportexcludeenv}
...
### `process.report.excludeEnv` {#processreportsignal}
...
### `process.report.signal` {#processreportwritereportfilename-err}
...
### `process.report.writeReport([filename][, err])` {#processresourceusage}
...
## `process.resourceUsage()` {#processsendmessage-sendhandle-options-callback}

**Added in: v12.6.0**

- Returns: [\<Object\>](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Object) the resource usage for the current process. All of these values come from the `uv_getrusage` call which returns a [`uv_rusage_t` struct](https://docs.libuv.org/en/v1.x/misc#c.uv_rusage_t). 
    - `userCPUTime` [\<integer\>](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#Number_type) maps to `ru_utime` computed in microseconds. It is the same value as [`process.cpuUsage().user`](/nodejs/api/process#processcpuusagepreviousvalue).
    - `systemCPUTime` [\<integer\>](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#Number_type) maps to `ru_stime` computed in microseconds. It is the same value as [`process.cpuUsage().system`](/nodejs/api/process#processcpuusagepreviousvalue).
...
## `process.send(message[, sendHandle[, options]][, callback])` {#processsetegidid}

**Added in: v0.5.9**

- `message` [\<Object\>](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Object)
- `sendHandle` [\<net.Server\>](/nodejs/api/net#class-netserver) | [\<net.Socket\>](/nodejs/api/net#class-netsocket)
...
If Node.js is spawned with an IPC channel, the `process.send()` method can be used to send messages to the parent process. Messages will be received as a [`'message'`](/nodejs/api/child_process#event-message) event on the parent's [`ChildProcess`](/nodejs/api/child_process#class-childprocess) object.

If Node.js was not spawned with an IPC channel, `process.send` will be `undefined`.

The message goes through serialization and parsing. The resulting message might not be the same as what is originally sent.

## `process.setegid(id)` {#processseteuidid}
...
The `process.setegid()` method sets the effective group identity of the process. (See [`setegid(2)`](http://man7.org/linux/man-pages/man2/setegid.2).) The `id` can be passed as either a numeric ID or a group name string. If a group name is specified, this method blocks while resolving the associated a numeric ID.
...
## `process.seteuid(id)` {#processsetgidid}
...
The `process.seteuid()` method sets the effective user identity of the process. (See [`seteuid(2)`](http://man7.org/linux/man-pages/man2/seteuid.2).) The `id` can be passed as either a numeric ID or a username string. If a username is specified, the method blocks while resolving the associated numeric ID.
...
## `process.setgid(id)` {#processsetgroupsgroups}
...
The `process.setgid()` method sets the group identity of the process. (See [`setgid(2)`](http://man7.org/linux/man-pages/man2/setgid.2).) The `id` can be passed as either a numeric ID or a group name string. If a group name is specified, this method blocks while resolving the associated numeric ID.
...
## `process.setgroups(groups)` {#processsetuidid}
...
## `process.setuid(id)` {#processsetsourcemapsenabledval}
...
The `process.setuid(id)` method sets the user identity of the process. (See [`setuid(2)`](http://man7.org/linux/man-pages/man2/setuid.2).) The `id` can be passed as either a numeric ID or a username string. If a username is specified, the method blocks while resolving the associated numeric ID.
...
## `process.setSourceMapsEnabled(val)` {#processsetuncaughtexceptioncapturecallbackfn}

**Added in: v16.6.0, v14.18.0**

::: warning [Stable: 1 - Experimental]
[Stable: 1](/nodejs/api/documentation#stability-index) [Stability: 1](/nodejs/api/documentation#stability-index) - Experimental: Use [`module.setSourceMapsSupport()`](/nodejs/api/module#modulesetsourcemapssupportenabled-options) instead.
:::
...
## `process.setUncaughtExceptionCaptureCallback(fn)` {#processsourcemapsenabled}
...
If such a function is set, the [`'uncaughtException'`](/nodejs/api/process#event-uncaughtexception) event will not be emitted. If `--abort-on-uncaught-exception` was passed from the command line or set through [`v8.setFlagsFromString()`](/nodejs/api/v8#v8setflagsfromstringflags), the process will not abort. Actions configured to take place on exceptions such as report generations will be affected too
...
Using this function is mutually exclusive with using the deprecated [`domain`](/nodejs/api/domain) built-in module.

## `process.sourceMapsEnabled` {#processstderr}

**Added in: v20.7.0, v18.19.0**

::: warning [Stable: 1 - Experimental]
[Stable: 1](/nodejs/api/documentation#stability-index) [Stability: 1](/nodejs/api/documentation#stability-index) - Experimental: Use [`module.getSourceMapsSupport()`](/nodejs/api/module#modulegetsourcemapssupport) instead.
:::

- [\<boolean\>](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#Boolean_type)

The `process.sourceMapsEnabled` property returns whether the [Source Map v3](https://sourcemaps.info/spec) support for stack traces is enabled.

## `process.stderr` {#processstderrfd}

- [\<Stream\>](/nodejs/api/stream#stream)

The `process.stderr` property returns a stream connected to `stderr` (fd `2`). It is a [`net.Socket`](/nodejs/api/net#class-netsocket) (which is a [Duplex](/nodejs/api/stream#duplex-and-transform-streams) stream) unless fd `2` refers to a file, in which case it is a [Writable](/nodejs/api/stream#writable-streams) stream.

`process.stderr` differs from other Node.js streams in important ways. See [note on process I/O](/nodejs/api/process#a-note-on-process-io) for more information.

### `process.stderr.fd` {#processstdin}

- [\<number\>](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#Number_type)

This property refers to the value of underlying file descriptor of `process.stderr`. The value is fixed at `2`. In [`Worker`](/nodejs/api/worker_threads#class-worker) threads, this field does not exist.

## `process.stdin` {#processstdinfd}

- [\<Stream\>](/nodejs/api/stream#stream)

The `process.stdin` property returns a stream connected to `stdin` (fd `0`). It is a [`net.Socket`](/nodejs/api/net#class-netsocket) (which is a [Duplex](/nodejs/api/stream#duplex-and-transform-streams) stream) unless fd `0` refers to a file, in which case it is a [Readable](/nodejs/api/stream#readable-streams) stream.

For details of how to read from `stdin` see [`readable.read()`](/nodejs/api/stream#readablereadsize).

As a [Duplex](/nodejs/api/stream#duplex-and-transform-streams) stream, `process.stdin` can also be used in "old" mode that is compatible with scripts written for Node.js prior to v0.10. For more information see [Stream compatibility](/nodejs/api/stream#compatibility-with-older-nodejs-versions).
...
### `process.stdin.fd` {#processstdout}

- [\<number\>](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#Number_type)

This property refers to the value of underlying file descriptor of `process.stdin`. The value is fixed at `0`. In [`Worker`](/nodejs/api/worker_threads#class-worker) threads, this field does not exist.

## `process.stdout` {#processstdoutfd}

- [\<Stream\>](/nodejs/api/stream#stream)

The `process.stdout` property returns a stream connected to `stdout` (fd `1`). It is a [`net.Socket`](/nodejs/api/net#class-netsocket) (which is a [Duplex](/nodejs/api/stream#duplex-and-transform-streams) stream) unless fd `1` refers to a file, in which case it is a [Writable](/nodejs/api/stream#writable-streams) stream.
...
`process.stdout` differs from other Node.js streams in important ways. See [note on process I/O](/nodejs/api/process#a-note-on-process-io) for more information.

### `process.stdout.fd` {#a-note-on-process-i/o}

- [\<number\>](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#Number_type)

This property refers to the value of underlying file descriptor of `process.stdout`. The value is fixed at `1`. In [`Worker`](/nodejs/api/worker_threads#class-worker) threads, this field does not exist.

### A note on process I/O {#processthrowdeprecation}
...
Synchronous writes avoid problems such as output written with `console.log()` or `console.error()` being unexpectedly interleaved, or not written at all if `process.exit()` is called before an asynchronous write completes. See [`process.exit()`](/nodejs/api/process#processexitcode) for more information.

*<strong>Warning</strong>*: Synchronous writes block the event loop until the write has completed. This can be near instantaneous in the case of output to a file, but under high system load, pipes that are not being read at the receiving end, or with slow terminals or file systems, it's possible for the event loop to be blocked often enough and long enough to have severe negative performance impacts. This may not be a problem when writing to an interactive terminal session, but consider this particularly careful when doing production logging to the process output streams.

To check if a stream is connected to a [TTY](/nodejs/api/tty#tty) context, check the `isTTY` property.
...
false
```
See the [TTY](/nodejs/api/tty#tty) documentation for more information.

## `process.throwDeprecation` {#processtitle}

**Added in: v0.9.12**

- [\<boolean\>](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#Boolean_type)

The initial value of `process.throwDeprecation` indicates whether the `--throw-deprecation` flag is set on the current Node.js process. `process.throwDeprecation` is mutable, so whether or not deprecation warnings result in errors may be altered at runtime. See the documentation for the [`'warning'` event](/nodejs/api/process#event-warning) and the [`emitWarning()` method](/nodejs/api/process#processemitwarningwarning-type-code-ctor) for more information.
...
```
## `process.title` {#processtracedeprecation}
...
## `process.traceDeprecation` {#processumask}
...
## `process.umask()` {#processumaskmask}
...
::: danger [Stable: 0 - Deprecated]
[Stable: 0](/nodejs/api/documentation#stability-index) [Stability: 0](/nodejs/api/documentation#stability-index) - Deprecated. Calling `process.umask()` with no argument causes the process-wide umask to be written twice. This introduces a race condition between threads, and is a potential security vulnerability. There is no safe, cross-platform alternative API.
:::
...
## `process.umask(mask)` {#processuptime}
...
In [`Worker`](/nodejs/api/worker_threads#class-worker) threads, `process.umask(mask)` will throw an exception.

## `process.unref(maybeRefable)` {#processversion}
...
## `process.uptime()` {#processversions}
...
- `1` **Uncaught Fatal Exception**: There was an uncaught exception, and it was not handled by a domain or an [`'uncaughtException'`](/nodejs/api/process#event-uncaughtexception) event handler.
...
- `7` **Internal Exception Handler Run-Time Failure**: There was an uncaught exception, and the internal fatal exception handler function itself threw an error while attempting to handle it. This can happen, for example, if an [`'uncaughtException'`](/nodejs/api/process#event-uncaughtexception) or `domain.on('error')` handler throws an error.
